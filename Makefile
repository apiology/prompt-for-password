...
quicktypecheck: ## Use dmypy for cached mypy runs.
	@dmypy run --timeout 300 *.py tests prompt_for_password
...
typecheck: ## run mypy against project
	mypy --cobertura-xml-report typecover --html-report typecover prompt_for_password
	mypy tests
...
test: ## run tests quickly with the default Python
	@coverage run --source prompt_for_password -m pytest
...
docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/prompt_for_password.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ prompt_for_password
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
release: dist ## package and upload a release
	...
	twine upload -u $$(with-op op get item 'PyPI - test' --fields username) -p $$(with-op op get item 'PyPI - test' --fields password) dist/prompt-for-password-$${new_version:?}.tar.gz -r testpypi; \
	twine upload -u $$(with-op op get item 'PyPI' --fields username) -p $$(with-op op get item 'PyPI' --fields password) dist/prompt-for-password-$${new_version:?}.tar.gz -r pypi
